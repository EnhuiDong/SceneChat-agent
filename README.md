...
12. 角色 Intent 会声明 `addressed_to`、`reply_to_event_id`、`conversation_move` 和紧迫度。调度器优先安排被直接提问、请求或质疑且尚未回应的角色；情绪、当前对话目标、待履行承诺和未回答问题会随会话持久化，但不具备修改世界状态的权限。
13. 长期记忆不再保存每轮泛泛的 `private_reason`，而是从结构化候选中保留主张、线索、承诺、关系证据、信息披露和关键决定，并按当前对话对象与重要度筛选；每个阶段或固定间隔生成确定性摘要。Intent 提交前还会检查角色自我重复、复述上一人、无视待回应问题、超长独白和不可见秘密泄漏，至多进行一次定向重试；若重试后仍无视必须回应的提问，则使用不泄密的显式回避作为降级响应，避免空轮次吞掉对话义务。
14. 被直接提问的角色仍拥有最高回应优先级；被点名但未被直接询问的角色会得到有时效的可选插话机会，并受连续发言冷却约束。关系状态使用信任、怀疑、亲近三轴，每次变化必须引用角色可见的历史事件、附带私人证据说明且单轴单轮限幅为 ±0.2。质问、回避、回答和主动披露会以小幅、可恢复的方式影响披露压力，不会强制角色公开全部秘密。
