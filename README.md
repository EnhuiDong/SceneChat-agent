...
13. 长期记忆不再保存每轮泛泛的 `private_reason`，而是从结构化候选中保留主张、线索、承诺、关系证据、信息披露和关键决定，并按当前对话对象与重要度筛选；每个阶段或固定间隔生成确定性摘要。Intent 提交前还会检查角色自我重复、复述上一人、无视待回应问题、超长独白和不可见秘密泄漏，至多进行一次定向重试。
